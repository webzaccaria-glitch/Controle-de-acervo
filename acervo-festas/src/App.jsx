...
function printChecklist(rental,inventory) {
  ...
  const makeRows=(mode)=>saida.map(it=>{
    ...
    const catLabel=inv?(CATS.find(c=>c.key===inv.mainCategory)||{label:inv.mainCategory||''}).label:''
...
<strong>Em caso de avaria, dano, quebra ou perda</strong> das peças locadas, o locatário pagará o valor de <strong>100% referente ao valor do item novo</strong>, devendo ser pago <strong>no ato da entrega do material</strong>, além do custo de reparo ou reposição quando aplicável.<br/><br/>
O não cumprimento do prazo de devolução implica cobrança adicional de <strong style="color:#c0392b">${R$(rental.dailyRate||50)} por dia de atraso</strong>, contados a partir da data de retorno prevista.</p></div>
...
export default function App() {
  ...
  const handleTokenResponse = async (response) => {
    ...
    try {
      ...
      const [invId,rentId,subcatId] = await Promise.all([
      ...
      setDriveIds({folder:folderId,inventory:invId,rentals:rentId,subcategories:subcatId})

      setLoadingMsg('Carregando seus dados…')
      const [inv,rent,subcats] = await Promise.all([
        readFile(tk,invId),
        readFile(tk,rentId),
  ...
  const logout = () => {
    ...
    setInventory([]); setRentals([]); setSubcats(EMPTY_SUBCATS); setAdminUnlocked(false)
  }
  ...
  return(
    <div style={{minHeight:'100vh',background:'#0f0e17',fontFamily:"'Source Sans 3', sans-serif"}}>
      ...
      <div style={{padding:'20px',maxWidth:1100,margin:'0 auto'}}>
        {tab==='rentals'&&<RentalsTab rentals={rentals} inventory={inventory} subcategories={subcategories} onAdd={addRental} onUpdate={updateRental} onDelete={deleteRental} onStatusChange={updateStatus}/>}
        {tab==='admin'&&(
          adminUnlocked?(
            <div>
              ...
              {adminSub==='inventory'&&<InventoryTab inventory={inventory} rentals={rentals} subcategories={subcategories} onAdd={addInventory} onUpdate={updateInventory} onDelete={deleteInventory} onAddSubcat={addSubcat} onDeleteSubcat={deleteSubcat}/>}
...
// ── Inventory Tab ─────────────────────────────────────────────────────────────
function InventoryTab({inventory,rentals,subcategories,onAdd,onUpdate,onDelete,onAddSubcat,onDeleteSubcat}) {
  const [expanded,setExpanded]   = useState({tecidos:true,'acrílicos':true,metálicos:true,madeira:true})
  const [showForm,setShowForm]   = useState(false)
  const [editing,setEditing]     = useState(null)
  const [saving,setSaving]       = useState(false)
  const [presetCat,setPresetCat] = useState('')
  const [presetSub,setPresetSub] = useState('')
  const [subcatModal,setSubcatModal] = useState(null) // category key
  ...
  const openNew=(cat='',sub='')=>{
    setForm({...blank,mainCategory:cat||'madeira',subCategory:sub||''})
  ...
  return(
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(120px,1fr))',gap:10,marginBottom:24}}>
        {[['📦','Total de Tipos',inventory.length],['🔢','Total de Peças',totalItems],['🎨','Pintados',inventory.filter(i=>i.painted).length],
          ...CATS.map(c=>[c.icon,c.label,inventory.filter(i=>i.mainCategory===c.key).length])
        ].map(([ic,label,val])=>(
          <div key={label} className="card" style={{textAlign:'center',padding:'10px 8px'}}>
            <div style={{fontSize:18}}>{ic}</div>
            <div style={{fontSize:18,fontWeight:700,color:'#d4a843',fontFamily:'serif'}}>{val}</div>
      ...
      {/* Category sections */}
      {CATS.map(cat=>{
        const catItems = inventory.filter(i=>i.mainCategory===cat.key)
        ...
        return(
          <div key={cat.key} className="cat-section">
            {/* Category header */}
            <div className="cat-header" style={{background:`linear-gradient(135deg,${cat.bg},${cat.bg}dd)`}}
              ...
              <div style={{flex:1}}>
                <div style={{fontWeight:700,color:'#fff',fontSize:15,letterSpacing:0.5}}>{cat.label.toUpperCase()}</div>
                <div style={{fontSize:11,color:'#ffffff80',marginTop:1}}>{catItems.length} item(s) · {catItems.reduce((a,i)=>a+Number(i.quantity||0),0)} unidades</div>
              </div>
              <div style={{display:'flex',gap:6,alignItems:'center'}} onClick={e=>e.stopPropagation()}>
                <button className="btn" style={{background:'#ffffff20',color:'#fff',border:'1px solid #ffffff30',fontSize:11,padding:'5px 10px'}}
                  onClick={e=>{e.stopPropagation();setSubcatModal(cat.key);setSubcatName('')}}>
                  + Sub-categoria
                </button>
                <button className="btn btn-gold" style={{fontSize:11,padding:'5px 10px'}}
                  onClick={e=>{e.stopPropagation();openNew(cat.key,'')}}>
                  + Item
                </button>
                <span style={{color:'#ffffff60',fontSize:16,marginLeft:4}}>{isOpen?'▾':'▸'}</span>
              </div>
            </div>
            ...
            {isOpen&&(
              <div className="cat-body">
                {catItems.length===0&&catSubs.length===0&&(
                  <div style={{textAlign:'center',padding:'20px 0',color:'#4a4060'}}>
                    <div style={{fontSize:28,marginBottom:8}}>{cat.icon}</div>
                    <div style={{fontSize:13}}>Nenhum item em {cat.label} ainda.</div>
                    <button className="btn btn-gold" style={{marginTop:12,fontSize:12}} onClick={()=>openNew(cat.key,'')}>+ Adicionar primeiro item</button>
                  </div>
                )}

                {/* Defined sub-categories first */}
                {catSubs.map(sub=>{
                  const subItems = groups[sub]||[]
                  return(
                    <div key={sub} className="subcat-group">
                      <div className="subcat-header">
                        <span style={{fontSize:13,fontWeight:600,color:`${cat.accent}`}}>📂 {sub}</span>
                        <div style={{display:'flex',gap:6}}>
                          <button className="btn btn-ghost" style={{fontSize:11,padding:'3px 8px'}} onClick={()=>openNew(cat.key,sub)}>+ Item</button>
                          <button className="btn btn-danger" style={{fontSize:11,padding:'3px 6px'}} onClick={()=>window.confirm(`Excluir sub-categoria "${sub}"?`)&&onDeleteSubcat(cat.key,sub)}>🗑</button>
                        </div>
                      </div>
                      {subItems.length===0&&<div style={{fontSize:12,color:'#4a4060',padding:'6px 10px',fontStyle:'italic'}}>Nenhum item nesta sub-categoria ainda.</div>}
                      {subItems.map(it=><ItemRow key={it.id} item={it} catAccent={cat.accent} rentals={rentals} onEdit={openEdit} onDelete={handleDelete}/>)}
                    </div>
                  )
                })}
                ...
                {(groups['']||[]).length>0&&(
                  <div className="subcat-group">
                    {catSubs.length>0&&(
                      <div className="subcat-header">
                        <span style={{fontSize:12,color:'#6a6080',fontStyle:'italic'}}>Sem sub-categoria</span>
                        <button className="btn btn-ghost" style={{fontSize:11,padding:'3px 8px'}} onClick={()=>openNew(cat.key,'')}>+ Item</button>
                      </div>
                    )}
                    {(groups['']||[]).map(it=><ItemRow key={it.id} item={it} catAccent={cat.accent} rentals={rentals} onEdit={openEdit} onDelete={handleDelete}/>)}
                  </div>
                )}
              </div>
            )}
      ...
      {subcatModal&&(
        <div className="modal-overlay" onClick={()=>setSubcatModal(null)}>
          <div style={{background:'#16152a',border:'1px solid #2e2b4a',borderRadius:16,padding:28,width:'100%',maxWidth:380}} onClick={e=>e.stopPropagation()}>
            <div style={{fontFamily:'serif',fontSize:18,color:'#d4a843',marginBottom:16}}>
              + Nova Sub-categoria em {CATS.find(c=>c.key===subcatModal)?.label}
            </div>
            <label>Nome da Sub-categoria</label>
            <input value={subcatName} onChange={e=>setSubcatName(e.target.value)}
              onKeyDown={e=>e.key==='Enter'&&handleAddSubcat(subcatModal)}
              placeholder="Ex: Voal, Mesa Rústica, Ferro Forjado…" style={{marginBottom:16}}/>
        ...
        <div className="modal-overlay" onClick={close}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="form-grid">
              <div style={{gridColumn:'1/-1'}}><label>Nome do Item *</label><input value={form.name} onChange={e=>set('name',e.target.value)} placeholder="Ex: Mesa Provençal, Lustre…"/></div>
              <div>
    ...
    <div className="item-row">
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
          ...
          {item.painted&&<span className="tag" style={{background:'#1b3a1b',color:'#6ee76e',fontSize:10}}>🎨</span>}
          {semPreco
            ?<span className="tag" style={{background:'#3a2800',color:'#f0a020',border:'1px solid #8a5500',fontSize:10}}>⚠ Sem preço</span>
            :<span className="tag" style={{background:'#1c2a1c',color:'#a0d8a0',fontSize:10}}>💰 {R$(item.rentalPrice)}</span>
          }
          <span className="tag" style={{background:'#1a1929',color:'#6a6080',border:'1px solid #2e2b4a',fontSize:10}}>🔄 {useCount}x</span>
        </div>
        {pct!==null&&(
      ...
      <div style={{textAlign:'center',minWidth:50,borderLeft:'1px solid #2e2b4a',paddingLeft:12}}>
        <div style={{fontSize:20,fontWeight:800,color:catAccent,fontFamily:'serif'}}>{item.quantity}</div>
...
// ── Rentals Tab ───────────────────────────────────────────────────────────────
function RentalsTab({rentals,inventory,subcategories,onAdd,onUpdate,onDelete,onStatusChange}) {
  const [showForm,setShowForm]=useState(false)
            ...
            {inventory.length===0?<div style={{background:'#1a1929',border:'1px solid #2e2b4a',borderRadius:8,padding:14,color:'#6a6080',fontSize:13,marginBottom:12}}>⚠ Nenhum item no acervo.</div>:(()=>{
              ...
              <div style={{background:'#0f0e17',border:'1px solid #2e2b4a',borderRadius:10,overflow:'hidden',marginBottom:4}}>
                ...
                {form.items.map(it=>{
                  ...
                  const catInfo=CATS.find(c=>c.key===inv?.mainCategory)||{icon:'',label:inv?.mainCategory||''}
                  return(
  ...
  return(
    <div>
      ...
      {inventory.filter(i=>Number(i.replacementCost)>0).length===0?(
        <div className="card" style={{padding:20,color:'#6a6080',fontSize:13}}>Nenhum item com custo de reposição cadastrado. Vá em <strong style={{color:'#d4a843'}}>Administrativo → Acervo</strong> e informe o custo de reposição e vida útil.</div>
      ):(
        <div style={{background:'#16152a',border:'1px solid #2e2b4a',borderRadius:12,overflow:'hidden'}}>
          ...
          {inventory.filter(i=>Number(i.replacementCost)>0).map((it,idx)=>{
            ...
            const catInfo=CATS.find(c=>c.key===it.mainCategory)||{icon:'',label:it.mainCategory||''}
            ...
            return(
              <div key={it.id} style={{display:'grid',gridTemplateColumns:'1fr 70px 90px 90px 100px 90px',alignItems:'center',padding:'12px 16px',borderTop:'1px solid #1e1d35',background:idx%2===0?'transparent':'#0f0e1710'}}>
                <div><div style={{color:'#e8dfc8',fontSize:13,fontWeight:600}}>{it.name}</div><div style={{fontSize:11,color:'#6a6080'}}>{catInfo.icon} {catInfo.label}</div></div>
