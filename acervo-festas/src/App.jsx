...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}body{font-family:'Source Sans 3',sans-serif;color:#1a1a2e;padding:32px 40px;font-size:13px;}
  ...
  .fin{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin:8px 0 18px;}.fc{border-radius:6px;padding:10px 14px;text-align:center;}.fc label{font-size:10px;text-transform:uppercase;letter-spacing:1px;color:#8a7a50;display:block;margin-bottom:4px;}.fc .v{font-size:16px;font-weight:700;}
  .ft{background:#faf8f2;border:1px solid #e0d8c0;}.fp{background:#f0fff4;border:1px solid #b7e4c7;}.fp .v{color:#2d6a4f;}.fd{background:#fff5f5;border:1px solid #f5c0c0;}.fd .v{color:#c0392b;}.fq{background:#f0fff4;border:1px solid #b7e4c7;}.fq .v{color:#2d6a4f;}
...
<div class="fin">
  ...
  <div class="fc ${debito>0?"fd":"fq"}"><label>${debito>0?"⚠ Valor em Débito":"✓ Quitado"}</label><span class="v">${R$(debito)}</span></div>
</div>
...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .btn-ghost { background:transparent; border:1px solid #2e2b4a; color:#b0a8c0; } .btn-ghost:hover { border-color:#d4a843; color:#d4a843; }
  .btn-danger{ background:#3d1c1c; color:#e05c5c; border:1px solid #5a2222; } .btn-danger:hover{ background:#5a2222; }
  ...
  .btn-blue  { background:linear-gradient(135deg,#1e3a5f,#0d2440); color:#a8d4ff; border:1px solid #2e4a6a; } .btn-blue:hover  { filter:brightness(1.2); transform:translateY(-1px); }
...
export default function App() {
  ...
  if (loading) return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", height:"100vh", background:"#0f0e17", gap:16 }}>
      <div style={{ color:"#d4a843", fontFamily:"serif", fontSize:24 }}>✦ Acervo de Festas</div>
  ...
  if (error) return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", height:"100vh", background:"#0f0e17", gap:16 }}>
      ...
      <button onClick={loadAll} style={{ background:"#d4a843", border:"none", borderRadius:8, padding:"10px 24px", cursor:"pointer", fontWeight:700 }}>Tentar novamente</button>
    </div>
  );
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#0f0e17", fontFamily:"'Source Sans 3', sans-serif" }}>
      ...
      <div style={{ padding:"24px", maxWidth:1100, margin:"0 auto" }}>
        {tab==="inventory"
          ? <InventoryTab inventory={inventory} onAdd={addInventory} onUpdate={updateInventory} onDelete={deleteInventory} />
          : <RentalsTab   rentals={rentals} inventory={inventory} onAdd={addRental} onUpdate={updateRental} onDelete={deleteRental} onStatusChange={updateRentalStatus} />}
      </div>
    </div>
  );
}
...
// ── Inventory Tab ─────────────────────────────────────────────────────────────
function InventoryTab({ inventory, onAdd, onUpdate, onDelete }) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    if (editing) {
      await onUpdate({ ...form, id:editing });
    } else {
      await onAdd({ ...form, id:uid() });
    }
    setSaving(false);
    close();
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Excluir este item?")) await onDelete(id);
  };
...
function RentalsTab({ rentals, inventory, onAdd, onUpdate, onDelete, onStatusChange }) {
  ...
  const [saving, setSaving]       = useState(false);
  const blank = { tenantName:"", address:"", phone:"", rentalDate:"", amountPaid:"", exitBy:"", items:[], status:"ativa", totalOrder:0 };
  const [form, setForm] = useState(blank);
  ...
  const handleSave = async () => {
    ...
    if (editingId) await onUpdate({ ...data, id:editingId });
    else           await onAdd({ ...data, id:uid() });
    setSaving(false);
    close();
  };
  ...
  return (
    <div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:12, marginBottom:24 }}>
        {[["🎪","Total de Locações",rentals.length,"#d4a843"],["✅","Locações Ativas",rentals.filter(r=>r.status==="ativa").length,"#6ee76e"],["💰","Faturamento Total",R$(totalFat),"#d4a843"],["⚠️","Débito em Aberto",R$(totalDeb),totalDeb>0?"#e05c5c":"#6ee76e"]].map(([ic,label,val,clr])=>(
                ...
                <div style={{ display:"flex", alignItems:"flex-start", gap:16, flexWrap:"wrap" }}>
                  <div style={{ flex:1, minWidth:220 }}>
                    ...
                    <div style={{ fontSize:12, color:"#8a7a9a", marginTop:5, display:"flex", flexWrap:"wrap", gap:"2px 14px" }}>
                      <span>📍 {r.address||"—"}</span><span>📞 {r.phone||"—"}</span><span>📅 {fmtDate(r.rentalDate)}</span><span>🧑 {r.exitBy||"—"}</span>
                    </div>
      ...
      {showForm && (
        <div className="modal-overlay" onClick={close}>
          <div className="modal" style={{ maxWidth:700 }} onClick={e=>e.stopPropagation()}>
            ...
            {inventory.length===0 ? (
              <div style={{ background:"#1a1929", border:"1px solid #2e2b4a", borderRadius:8, padding:14, color:"#6a6080", fontSize:13, marginBottom:12 }}>⚠ Nenhum item no acervo. Adicione na aba <strong>📦 Acervo</strong>.</div>
            ) : (
            ...
            <div style={{ display:"flex", gap:10, justifyContent:"flex-end" }}>
              <button className="btn btn-ghost" onClick={close}>Cancelar</button>
              <button className="btn btn-gold" onClick={handleSave} disabled={saving}>
                {saving?"Salvando…":editingId?"💾 Salvar Ajustes":"✦ Registrar Locação"}
              </button>
            </div>
          </div>
        </div>
