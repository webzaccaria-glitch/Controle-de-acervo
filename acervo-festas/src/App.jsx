...
const EMPTY_SUBCATS = { tecidos:[], 'acrílicos':[], metálicos:[], madeira:[] }
            ...
            {inventory.length===0?<div style={{background:'#1a1929',border:'1px solid #2e2b4a',borderRadius:8,padding:14,color:'#6a6080',fontSize:13,marginBottom:12}}>⚠ Nenhum item no acervo.</div>:(
              <div style={{display:'flex',gap:8,marginBottom:10}}>
                <select value={sel.itemId} onChange={e=>setSel(p=>({...p,itemId:e.target.value}))} style={{flex:1}}>
                  ...
                  {CATS.map(c=>{
                    const catItems=inventory.filter(i=>i.mainCategory===c.key)
                    if(!catItems.length) return null
                    return <optgroup key={c.key} label={`${c.icon} ${c.label}`}>{catItems.map(i=><option key={i.id} value={i.id}>{i.name}{i.subCategory?` (${i.subCategory})`:''} — {i.quantity} disp.{i.rentalPrice?` — ${R$(i.rentalPrice)}/un`:''}</option>)}</optgroup>
                  })}
                </select>
