...
const calcOverdue = (r) => {
  ...
  const days = Math.max(0,Math.round((new Date(compareDate+'T12:00:00')-new Date(r.expectedReturnDate+'T12:00:00'))/86400000))
...
const calcOverdueDays = (r) => {
  ...
  const compareDate = r.actualReturnDate||(r.status==='ativa'?today:null)
  if (!compareDate) return 0
  return Math.max(0,Math.round((new Date(compareDate+'T12:00:00')-new Date(r.expectedReturnDate+'T12:00:00'))/86400000))
}
const getDepreciation = (item, rentals) => {
  ...
  const expectedUses=Number(item.expectedUses)||100, replacementCost=Number(item.replacementCost)||0
  const costPerUse=replacementCost/expectedUses, accumulated=useCount*costPerUse
  const remaining=Math.max(0,expectedUses-useCount), pct=Math.min(100,Math.round((useCount/expectedUses)*100))
    ...
    <div style={{minHeight:'100vh',background:'#0f0e17',fontFamily:"'Source Sans 3', sans-serif"}}>
      ...
      <div style={{background:'linear-gradient(135deg,#1a1929,#0f0e17)',borderBottom:'1px solid #2e2b4a',padding:'16px 24px',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
        ...
        <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
          {[['rentals','🎪 Locações'],['finance','💰 Finanças'],['inventory','📦 Acervo 🔒']].map(([k,l])=>(
...
function InventoryTab({inventory,rentals,onAdd,onUpdate,onDelete}) {
  ...
  const handlePin=()=>{if(checkPin(pinInput)){setUnlocked(true);setPinError('');setPinInput('')}else{setPinError('PIN incorreto.');setPinInput('')}}
  const handleChangePin=()=>{if(newPin.length<4){setPinError('PIN deve ter no mínimo 4 dígitos.');return}setPin(newPin);setNewPin('');setChanging(false);setPinError('');alert('PIN alterado!')}
  ...
  if(!unlocked) return(
    <div style={{display:'flex',alignItems:'center',justifyContent:'center',minHeight:400}}>
      <div className="card" style={{maxWidth:380,width:'100%',textAlign:'center',padding:36}}>
        ...
        {!changingPin?(
          <>
            <input type="password" inputMode="numeric" maxLength={8} value={pinInput} onChange={e=>setPinInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handlePin()} placeholder="Digite o PIN" style={{textAlign:'center',letterSpacing:6,fontSize:18,marginBottom:8}}/>
            {pinError&&<div style={{color:'#e05c5c',fontSize:12,marginBottom:8}}>{pinError}</div>}
            <button className="btn btn-gold" style={{width:'100%',justifyContent:'center',marginBottom:10}} onClick={handlePin}>Entrar</button>
            <button className="btn btn-ghost" style={{width:'100%',justifyContent:'center',fontSize:12}} onClick={()=>{setChanging(true);setPinError('')}}>🔑 Alterar PIN</button>
            <div style={{fontSize:11,color:'#4a4060',marginTop:16}}>PIN padrão: <code style={{color:'#d4a843'}}>1234</code></div>
          </>
        ):(
          <>
            <input type="password" inputMode="numeric" maxLength={8} value={pinInput} onChange={e=>setPinInput(e.target.value)} placeholder="PIN atual" style={{textAlign:'center',marginBottom:8}}/>
            <input type="password" inputMode="numeric" maxLength={8} value={newPin} onChange={e=>setNewPin(e.target.value)} placeholder="Novo PIN" style={{textAlign:'center',marginBottom:8}}/>
            {pinError&&<div style={{color:'#e05c5c',fontSize:12,marginBottom:8}}>{pinError}</div>}
            <button className="btn btn-gold" style={{width:'100%',justifyContent:'center',marginBottom:8}} onClick={()=>{if(!checkPin(pinInput)){setPinError('PIN atual incorreto.');return}handleChangePin()}}>Salvar novo PIN</button>
  ...
  return(
    <div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20,flexWrap:'wrap',gap:8}}>
        <span style={{fontSize:11,color:'#6ee76e',background:'#1b3a1b',border:'1px solid #2e4a2e',borderRadius:20,padding:'2px 10px'}}>🔓 Admin desbloqueado</span>
          ...
          <div className="modal" style={{maxWidth:700}} onClick={e=>e.stopPropagation()}>
            ...
            {formTotal>0&&(
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginTop:10}}>
                {[['TOTAL',R$(formTotal),'#d4a843','#0f0e17','#2e2b4a'],['PAGO',R$(Number(form.amountPaid||0)),'#6ee76e','#0f1a0f','#2e4a2e'],['DÉBITO',R$(formDebito),formDebito>0?'#e05c5c':'#6ee76e',formDebito>0?'#1a0f0f':'#0f1a0f',formDebito>0?'#5a2222':'#2e4a2e']].map(([l,v,c,bg,bd])=>(
  ...
  return(
    <div>
      ...
      {inventory.filter(i=>Number(i.replacementCost)>0).length===0?(
        <div className="card" style={{padding:20,color:'#6a6080',fontSize:13}}>Nenhum item com custo de reposição cadastrado.<br/><span style={{fontSize:12}}>Vá em <strong style={{color:'#d4a843'}}>📦 Acervo</strong> e informe o custo de reposição e vida útil de cada item.</span></div>
      ):(
